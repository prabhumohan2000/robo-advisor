...
describe('OrdersService', () => {
  ...
  beforeEach(async () => {
    ...
    // Create a test account for all tests
    const testAccount = accountsService.create({
      name: 'Test User',
  ...
  describe('create — BUY', () => {
    ...
    it('should deduct platform balance after a BUY', () => {
      ...
      const balance = accountsService.findOne(testAccountId).balance;
    ...
    it('should throw BadRequestException if platform balance is insufficient', () => {
      accountsService.withdraw(testAccountId, 99950); // Leave only $50
  ...
  describe('Decimal precision and edge cases', () => {
    it('should handle percentage calculations with precision', () => {
      ...
      // Total amount should be ACTUAL total (sum of actual item amounts), not requested amount
      // This ensures: totalAmount === sum of (shares × price) for all items
      expect(order.totalAmount).toBe(1000.3); // Actual: 333.4 + 666.9 = 1000.3
      const aapl = order.items.find((i) => i.symbol === 'AAPL')!;
      const tsla = order.items.find((i) => i.symbol === 'TSLA')!;

      // Each item's amount must equal shares × price (not allocated percentage)
      expect(aapl.amount).toBe(333.4); // 3.334 shares × $100
      expect(aapl.shares).toBe(3.334);
      expect(tsla.amount).toBe(666.9); // 6.669 shares × $100
      expect(tsla.shares).toBe(6.669);
    });
    ...
    it('should handle very small amounts with precision', () => {
      ...
      const order = service.create(dto);
      expect(order.totalAmount).toBe(0); // 0 shares × $150 = $0
      expect(order.items[0].shares).toBe(0); // Rounds down to 0
    });
  ...
  describe('Account-based balance and holdings', () => {
    it('should return initial platform balance with no holdings when no orders exist', () => {
      const result = accountsService.getBalance(testAccountId);
    ...
    it('should return correct holdings after BUY orders', () => {
      ...
      expect(result.balance).toBe(99000); // 100000 - 1000
    ...
    it('should update holdings after multiple BUY orders', () => {
      ...
      expect(result.balance).toBe(98500); // 100000 - 1000 - 500
    ...
    it('should reduce holdings and totalInvested after SELL orders', () => {
      ...
      expect(result.balance).toBe(99300); // 100000 - 1000 + 300
      expect(result.holdings).toHaveLength(1);
    ...
    it('should handle mixed BUY and SELL orders across multiple stocks', () => {
      ...
      expect(result.balance).toBe(98500);
      expect(result.holdings).toHaveLength(2);
      expect(result.totalInvested).toBe(1500);
    ...
    it('should track balance changes through BUY and SELL operations', () => {
      ...
      expect(result.balance).toBe(97500); // 100000 - 2000 - 1000 + 500
      expect(result.holdings).toHaveLength(2);
    ...
    it('should handle fractional shares correctly', () => {
      ...
      // Actual cost: 0.727 shares × $137.5 = $99.96 (not $100)
      // Balance: $100,000 - $99.96 = $99,900.04
      expect(result.balance).toBe(99900.04);
      expect(result.holdings).toHaveLength(1);
      expect(result.totalInvested).toBe(99.96); // Actual amount invested
